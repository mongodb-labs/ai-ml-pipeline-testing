...
This repository uses the `drivers/ai-ml-pipeline-testing` vault.  See the [wiki](https://wiki.corp.mongodb.com/spaces/DRIVERS/pages/239737385/Using+AWS+Secrets+Manager+to+Store+Testing+Secrets) for vault access.

The vault contains the credentialed URLs for each integration, and the secrets needed to access Azure OpenAI
models.  See the wiki for more information on [AI Model Access](https://wiki.corp.mongodb.com/spaces/DRIVERS/pages/260909214/DBX+AI+ML+Integrations+Contribution+Guide#DBXAI/MLIntegrationsContributionGuide-AIModelAccess).
